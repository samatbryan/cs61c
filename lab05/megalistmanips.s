...
mapLoop:
	...
	#loading address first line
	#need to multiply counter by 4 and then add to correctly offset
	#need to store on stack and load after jalr
	#load address of next node into a0 by lw not la
	#put address back to a1 by setting a1 to s1 instead

	 #add	t1, s0, x0	# load the address of the array of current node into t1
	lw t1, 0(s0) 		## new load address into t1
    lw	t2, 4(s0)		# load the size of the node's array into t2

    slli t3, t0, 2     # mul count by 4
    ...
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp) #save temps

	jalr s1		# call the function on that value.

    lw t0, 0(sp) #load temps
    lw t1, 4(sp)
    lw t2, 8(sp)
    addi sp, sp 12
